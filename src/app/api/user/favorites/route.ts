...
import { db } from '@/db'
import { coupletFavorites, couplets, coupletCategories, coupletContents, coupletVersions } from '@/db/schema/couplet'
import { users } from '@/db/schema/users'
import { authOptions } from '@/lib/authOptions'

// GET /api/user/favorites - 获取用户收藏的对联
export async function GET(request: NextRequest) {
  ...
  // 查询收藏的coupletId
  const favRows = await db.select({ coupletId: coupletFavorites.coupletId, addedAt: coupletFavorites.createdAt }).from(coupletFavorites).where(eq(coupletFavorites.userId, userId)).limit(limit).offset(offset)
  const coupletIds = favRows.map(row => row.coupletId)

  let favoriteCouplets: any[] = []
  if (coupletIds.length > 0) {
    favoriteCouplets = await db
      .select({
        id: couplets.id,
    ...
    // 添加对联内容
    favoriteCouplets = favoriteCouplets.map(couplet => ({
      ...couplet,
  ...
  // 合并收藏时间
  const favorites = favoriteCouplets.map(cp => {
    const fav = favRows.find(f => f.coupletId === cp.id)
    return { ...cp, addedAt: fav?.addedAt, model: cp.model }
  })
  ...
  return NextResponse.json({
    ...
    data: {
      ...
      pagination: {
        page,
        limit,
        total: favRows.length,
        totalPages: Math.ceil(favRows.length / limit),
        hasNext: page * limit < favRows.length,
        hasPrev: page > 1,
      },
    },
  })
