...
export default function ClientComicListPage({
  ...
  // 仅在交互时 fetch 数据（API路径保持不变，但数据映射为漫画格式）
  useEffect(() => {
        ...
        if (res.success && res.data) {
          setComics(
            (res.data.couplets || []).map((item: any) => ({
              ...
              author: item.author?.name || item.author?.username || t('unknown') || '未知',
              category: item.category || {},
              coverImage: item.coverImage,
              frameCount: item.contentCount || 4,
              viewCount: item.viewCount,
              likeCount: item.likeCount,
              createdAt: item.createdAt,
              tags: Array.isArray(item.tags) ? item.tags.map((t: any) => t.name || t) : [],
              style: item.model || 'manga',
              type: item.type,
                  ...
                  comics.map((comic, index) => {
                    return (
                      ...
                      >
                          ...
                          <div className="aspect-[4/3] bg-gradient-to-br from-purple-100 to-pink-100 dark:from-purple-900/30 dark:to-pink-900/30 rounded-2xl overflow-hidden border-2 border-purple-200 dark:border-purple-800 shadow-inner">
                            ...
                            <div className="absolute bottom-2 left-2 right-2 bg-black/70 backdrop-blur-sm rounded-xl p-3 text-white">
                              <div className="flex items-center justify-between text-xs">
                                <span>{comic.frameCount || 4} 帧</span>
                                <span>{comic.style || '未知风格'}</span>
                              </div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="absolute inset-0 bg-gradient-to-br from-yellow-400/5 via-transparent to-purple-600/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-3xl pointer-events-none" />
                      </div>
                    )
                  })
                )}
