...
import {
  ...
  InputNumber, Switch, Breadcrumb, Spin, Alert, Space, Card, Tag, Skeleton,
} from 'antd'
...
const EditPlanPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchPlanData = async () => {
      setLoading(true)
      try {
        const response = await fetch(`/api/admin/vip/plans/${id}`)
        ...
        form.setFieldsValue({
          ...
          isActive: data.status, // 映射status到isActive
  ...
  if (loading) return <PlanFormSkeleton />
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <h1 style={{ margin: 0, fontSize: 28, fontWeight: 600 }}>编辑VIP套餐</h1>
        <p style={{ margin: '8px 0 0 0', opacity: 0.9 }}>修改套餐信息、价格和功能特性</p>
      </div>
...
export default withPagePermission(EditPlanPage, {
